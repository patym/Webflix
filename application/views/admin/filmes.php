...
<div class="col-lg-6">
  <div class="jumbotron text-center" style="margin-top: 1vh;">
    ...
    <form  method="post" action="<?= URL ?>Admin/filmes" id="formFilme">
    ...
    <tbody>
      <?php  foreach ($this->resultadoBusca as $filme) {?>
        <tr>
          ...
          <td><?= $filme->year ?></td>
          <td><?= $filme->actors ?></td>
          <td><button class="btn btn-primary btnImage" value="<?= $filme->poster ?>" id="<?= $filme->title ?>"><i class="fa fa-external-link"></i></button></td>
        </tr>
      <?php }?>
    </tbody>
  </table>
